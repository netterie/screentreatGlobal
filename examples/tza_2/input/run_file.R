...
stop()
controlTreats <- return_value_from_id(id=control_treatments, 
                                     df=treat_chars,
                                     value='tx')
SSids <- return_value_from_id(id=control_notreat_rows, 
                                     df=control_notreat,
                                     value='SSid')
cases <- t(sapply(times, FUN=function(x) {
                      browser()
                    cases <- ageclin<=(ageentry+x)
                    round(table(SSids[cases])/ncol(cases))
                                }))
rownames(cases) <- paste('Year', times)
cases <- cbind(cases, rowSums(cases[,grepl('ER\\+', colnames(cases))]))
cases <- cbind(cases, rowSums(cases[,1:nrow(control_notreat)]))
colnames(cases)[(nrow(control_notreat)+1):(nrow(control_notreat)+2)] <- 
    c('Total ER+', 'Total')
...
if (1==1) {
    ...
    msurv_df <- round(as.matrix(do.call('rbind', msurvs)),2)
    msurvScr_df <- do.call('rbind', msurvsScr)
    msurv_table <- cbind(base_msurvs[,2:3],
